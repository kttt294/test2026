...
def test_btc_six_step_example():
    ...
    trace = []
    final = sim.apply_day(state, orders, trace=trace)
    assert state == before
...
def test_order_and_state_list_order_do_not_change_tie_break():
    sim, state, orders = example()
    expected = sim.apply_day(state, orders)
    sim.traffic.reset()
    state.my_agents.reverse()
    actual = sim.apply_day(state, list(reversed(orders)))
...
def test_refill_needs_a_reflection_step(steps, fuel):
    ...
    final = sim.apply_day(state, orders)
...
def test_swapping_adjacent_cells_is_not_refueling():
    ...
    final = sim.apply_day(state, [DayOrder(1, [AgentAction('move', 2)]),
...
def test_empty_fuel_patrol_collects_again_next_day_without_moving():
    ...
    for day in (1, 2):
        state = sim.apply_day(state, [DayOrder(1, [AgentAction('stay')])])
